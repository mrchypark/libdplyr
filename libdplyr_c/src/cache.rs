...
impl SimpleTranspileCache {
    ...
    pub fn get_cache_stats() -> String {
        let (cache_info, metrics) = REQUEST_CACHE.with(|cache| {
            ...
            let total_processing_time: u64 = cache.iter()
                .map(|(_, result)| result.processing_time_us)
                .sum();
            
            let total_access_count: u64 = cache.iter()
                .map(|(_, result)| result.access_count)
                .sum();
            ...
            let cache_info = (cache.len(), cache.cap().get(), avg_processing_time, avg_access_count);
...
mod tests {
    ...
    fn test_cache_functionality() {
        ...
        let result1 = SimpleTranspileCache::get_or_transpile(
            "select(col1)", 
            &options, 
            |_code, _opts| {
        ...
        let call_count_clone = call_count.clone();
        let result2 = SimpleTranspileCache::get_or_transpile(
            "select(col1)", 
            &options, 
            |_code, _opts| {
                call_count_clone.fetch_add(1, Ordering::SeqCst);
    ...
    fn test_lru_eviction() {
        ...
        for i in 0..100 {
            ...
            let _ = SimpleTranspileCache::get_or_transpile(
                &code,
                &options,
                |_code, _opts| Ok(format!("SELECT col{} FROM table", i))
            );
        }
        ...
        let _ = SimpleTranspileCache::get_or_transpile(
            "select(new_col)",
            &options,
            |_code, _opts| Ok("SELECT new_col FROM table".to_string())
        );
    ...
    fn test_ffi_cache_functions() {
        ...
        let _ = SimpleTranspileCache::get_or_transpile(
            "select(col0)",
            &options,
            |_code, _opts| Ok("SELECT col0 FROM table".to_string())
        );
        ...
        let stats_str = unsafe {
            std::ffi::CStr::from_ptr(stats_ptr).to_string_lossy()
        };
    ...
    fn test_debug_logging_functions() {
        ...
        for i in 0..3 {
            ...
            let _ = SimpleTranspileCache::get_or_transpile(
                &code,
                &options,
                |_code, _opts| Ok(format!("SELECT test_col{} FROM table", i))
            );
        }
    ...
    fn test_cache_performance_warning() {
        ...
        for i in 0..25 {
            ...
            let _ = SimpleTranspileCache::get_or_transpile(
                &code,
                &options,
                |_code, _opts| Ok(format!("SELECT unique_col{} FROM table", i))
            );
        }
...
pub extern "C" fn dplyr_cache_log_stats_detailed(prefix: *const c_char, include_timestamp: bool) {
    ...
    eprintln!("{}{}: {} (hit_rate: {:.2}%, effective: {})", 
        timestamp_str, prefix_str, stats, hit_rate * 100.0, 
        SimpleTranspileCache::is_cache_effective());
    
    // R10-AC2: Additional debug information in detailed mode
    if metrics.hits + metrics.misses > 0 {
        eprintln!("{}CACHE_PERFORMANCE: avg_processing_time: {}μs, cache_overhead: {}μs", 
            timestamp_str,
            if metrics.misses > 0 { metrics.total_processing_time_us / metrics.misses } else { 0 },
            if metrics.hits + metrics.misses > 0 { 
...
pub extern "C" fn dplyr_cache_log_performance_warning() -> bool {
    ...
    if metrics.hits + metrics.misses > 0 {
        ...
        if avg_cache_overhead > avg_processing_time / 10 {
            warnings.push(format!("High cache overhead: {}μs vs {}μs processing", 
    ...
    if metrics.evictions > metrics.hits / 2 {
        warnings.push(format!("Excessive evictions: {} ({}% of hits)", 
            metrics.evictions, 
            if metrics.hits > 0 { metrics.evictions * 100 / metrics.hits } else { 0 }
        ));
    }
