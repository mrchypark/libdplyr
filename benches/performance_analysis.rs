...
fn benchmark_memory_usage_estimation(c: &mut Criterion) {
    ...
    let test_cases = vec![
        ...
        (
            "medium",
            "select(a, b, c, d) %>% filter(a > 1 & b < 10) %>% arrange(desc(c))",
        ),
        (
            "large",
            r#"
...
fn benchmark_scaling_analysis(c: &mut Criterion) {
    ...
    for count in column_counts {
        ...
        group.bench_with_input(
            ...
            &query,
            |b, query| b.iter(|| transpiler.transpile(black_box(query.as_str()))),
        );
    }
...
fn benchmark_dialect_performance_characteristics(c: &mut Criterion) {
    ...
    let dialects = vec![
        ...
        (
            "mysql",
            Box::new(MySqlDialect) as Box<dyn libdplyr::SqlDialect>,
        ),
        (
            "sqlite",
            Box::new(SqliteDialect) as Box<dyn libdplyr::SqlDialect>,
        ),
    ];
    ...
    for (query_name, query) in &test_queries {
        for (dialect_name, dialect) in &dialects {
            ...
            group.bench_with_input(
                ...
                query,
                |b, query| b.iter(|| transpiler.transpile(black_box(query))),
            );
        }
    }
