...
use libdplyr::cli::stdin_reader::{StdinReader, StdinConfig, StdinError};
...
fn test_stdin_error_from_io_error() {
    ...
    assert!(stdin_error.to_string().contains("Failed to read from stdin"));
...
fn test_stdin_reader_default() {
    ...
    assert_eq!(reader1.config().max_input_size, reader2.config().max_input_size);
}
