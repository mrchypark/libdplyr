...
use libdplyr::cli::json_output::{
    ErrorInfo, InputInfo, JsonOutputFormatter, MetadataBuilder, ProcessingStats,
};
...
fn test_json_deserialization() {
    ...
    let json = formatter
        .format_success("SELECT * FROM users", metadata)
        .unwrap();
...
fn test_metadata_completeness() {
    ...
    let formatter = JsonOutputFormatter::pretty();
    let json = formatter
        .format_success("SELECT COUNT(*) FROM data", metadata)
        .unwrap();
