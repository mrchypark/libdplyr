...
use libdplyr::cli::signal_handler::{utils, SignalHandler};
...
fn test_signal_handler_creation_cross_platform() {
    ...
    assert!(
        handler.is_ok(),
        "Signal handler creation should work on all platforms"
    );

    let handler = handler.unwrap();
    assert!(
        !handler.should_shutdown(),
        "Handler should not indicate shutdown initially"
    );
    assert!(
        !handler.pipe_closed(),
        "Handler should not indicate pipe closed initially"
    );
}
...
fn test_signal_wait_timeout_cross_platform() {
    ...
    assert_eq!(
        result,
        libdplyr::cli::signal_handler::SignalWaitResult::Timeout
    );
}
...
fn test_ignore_sigpipe_cross_platform() {
    ...
    assert!(
        result.is_ok(),
        "ignore_sigpipe should work on all platforms"
    );
}
...
fn test_stdin_reader_creation_cross_platform() {
    ...
    assert!(
        reader_with_signals.is_ok(),
        "StdinReader with signals should work on all platforms"
    );
}
...
mod platform_specific_tests {
    ...
    mod windows_tests {
        ...
        fn test_windows_signal_handling() {
            ...
            assert!(
                result.is_ok(),
                "SIGPIPE handling should be no-op on Windows"
            );
        }
...
mod integration_tests {
    ...
    fn test_cross_platform_cli_execution() {
        ...
        assert!(
            output.is_ok(),
            "CLI should be executable on current platform"
        );

        let output = output.unwrap();
        assert!(
            output.status.success(),
            "CLI help should execute successfully"
        );
    }
...
mod performance_tests {
    ...
    fn test_signal_handler_performance() {
        ...
        assert!(
            ...
            "Signal handler creation should be fast"
        );
    }
    ...
    fn test_pipeline_detection_performance() {
        ...
        assert!(
            ...
            "Pipeline detection should be fast"
        );
    }
}
