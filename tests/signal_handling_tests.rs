...
fn test_signal_handler_creation() {
    ...
    assert!(
        handler.is_ok(),
        "Signal handler should be created successfully"
    );
    ...
    assert!(
        !handler.pipe_closed(),
        "Pipe should not be closed initially"
    );
}
...
fn test_signal_aware_processor_creation() {
    ...
    assert!(
        processor.is_ok(),
        "Signal-aware processor should be created successfully"
    );
}
...
fn test_utils_functions() {
    ...
    if cfg!(unix) {
        ...
        assert!(
            !utils::is_unix_like(),
            "Should not detect Unix-like system on non-Unix"
        );
    }
...
fn test_stdin_reader_with_signal_handling() {
    ...
    assert!(
        reader.is_ok(),
        "Signal-aware stdin reader should be created successfully"
    );
}
...
mod unix_integration_tests {
    ...
    fn test_sigpipe_handling() {
        ...
        assert!(
            !stdout.is_empty(),
            "Should produce some output before pipe closes"
        );
    }
    ...
    fn test_interrupt_handling() {
        ...
        assert!(
            handler.is_ok(),
            "Signal handler should initialize on Unix systems"
        );
    }
    ...
    fn test_pipeline_environment_detection() {
        ...
        assert!(
            output.status.success(),
            "Command should succeed in pipeline"
        );
    }
}
...
fn test_cli_with_signal_handling_integration() {
    ...
    assert!(
        output.status.success(),
        "CLI should handle stdin input successfully"
    );
