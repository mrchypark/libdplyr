...
use crate::{Transpiler, SqlDialect};
...
impl RegressionDetector {
    ...
    pub fn check_regression(&self, current_stats: &BatchPerformanceStats) -> Vec<String> {
        ...
        if let Some(baseline) = &self.baseline_stats {
            ...
            let time_ratio = current_stats.avg_time.as_secs_f64() / baseline.avg_time.as_secs_f64().max(0.001);
            if time_ratio > 1.1 {
            ...
            let throughput_ratio = current_stats.avg_throughput / baseline.avg_throughput.max(0.001);
