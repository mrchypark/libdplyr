...
use crate::cli::signal_handler::{SignalHandler, SignalError, ProcessingError};
use std::io::{self, Read, BufRead, BufReader, IsTerminal};
...
impl StdinReader {
    ...
    fn read_from_pipe_simple(&self, buffer: &mut String) -> StdinResult<()> {
        ...
        // Convert to string
        *buffer = String::from_utf8(temp_buffer)
            .map_err(|e| StdinError::ReadError(io::Error::new(
    ...
    fn read_from_pipe_with_signals(&self, buffer: &mut String, signal_handler: &SignalHandler) -> StdinResult<()> {
        let stdin = io::stdin();
                    ...
                    if let Some(max_size) = self.config.max_input_size {
                        if total_size > max_size {
                            return Err(StdinError::ReadError(io::Error::new(
                                io::ErrorKind::InvalidData,
                                format!("Input size ({} bytes) exceeds maximum allowed size ({} bytes)", 
                                       total_size, max_size)
                            )));
                        }
                    }
...
mod tests {
    ...
    fn test_stdin_error_display() {
        ...
        assert_eq!(error.to_string(), "No input provided and stdin is not piped");
